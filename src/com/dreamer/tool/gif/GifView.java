...
/**
 * GifView<br>
 * 本类可以显示�?个gif动画，其使用方法和android的其它view（如imageview)�?样�??<br>
    ...
    public void parseOk(boolean parseStatus,int frameIndex){
    	if(parseStatus){
    		if(gifDecoder != null){
    			...
    			case WAIT_FINISH:
    				if(frameIndex == -1){
    					if(gifDecoder.getFrameCount() > 1){     //当帧数大�?1时，启动动画线程
...
//	    			}
    ...
    public class GifDecoder extends Thread{
        ...
        private GifView action = null;
        
        
        private byte[] gifData = null;

        
        public GifDecoder(byte[] data,GifView act){
            gifData = data;
            action = act;
        }
        
        public GifDecoder(InputStream is,GifView act){
            in = is;
            action = act;
        }
