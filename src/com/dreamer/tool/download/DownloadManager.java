...
import com.dreamer.tool.string.StringTool;
...
public class DownloadManager extends Thread {
	...
	public static String FILE_ROOT = SDCARD_ROOT;
	...
	public static DownloadManager getDownloadManager(Context context) {
		if (downloadManager == null) {
			downloadManager = new DownloadManager(context, FILE_ROOT);
		}
	...
	public static DownloadManager getDownloadManager(Context context,
		...
		if (downloadManager == null) {
			downloadManager = new DownloadManager(context, rootPath);
		}
		return downloadManager;
	}

	private DownloadManager(Context context) {
		this(context, FILE_ROOT);
	}
	...
	private DownloadManager(Context context, String rootPath) {
		...
		DownLoadConfigUtil.getInstance().prepare(context);
		if (!StringTool.isEmpty(rootPath)) {
	...
	public void addHandler(String url) {
		if (getTotalhandlerCount() >= MAX_handler_COUNT) {
			if (mDownLoadCallback != null) {
				mDownLoadCallback.sendFailureMessage(url, "download fail");
			}
			return;
		}
	...
	public void checkUncompletehandlers() {
		...
		List<String> urlList = DownLoadConfigUtil.getInstance().getURLArray();
		if (urlList.size() >= 0) {
	...
	public synchronized void completehandler(AsyncHttpResponseHandler handler) {
		...
		if (mDownloadinghandlers.contains(handler)) {
			DownLoadConfigUtil.getInstance().clearURL(
					mDownloadinghandlers.indexOf(handler));
			mDownloadinghandlers.remove(handler);
	...
	private AsyncHttpResponseHandler newAsyncHttpResponseHandler(String url)
		...
		FileHttpResponseHandler handler = new FileHttpResponseHandler(url,
				rootPath, StringTool.getFileNameFromUrl(url)) {
			...
			public void onStart() {
				DownLoadConfigUtil.getInstance().storeURL(
						mDownloadinghandlers.indexOf(this), getUrl());
			}
