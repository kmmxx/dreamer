package com.dreamer.tool.string;

import java.io.UnsupportedEncodingException;

public class ChToSpell {
	...
	private static ChToSpell chToSpell;

	public static synchronized ChToSpell getInstance() {
		if(chToSpell==null)
			chToSpell = new ChToSpell();
		return chToSpell;
	}
...
//		System.out.println("pyvalue = "+result);
		return result;
	}
	//»ñÈ¡ººÓïÆ´ÒôÊ××ÖÄ¸
	public String convertFirstPy(String str) {
		String result = null;
		int ascii = getChsAscii(str);
		if (ascii > 0 && ascii < 160) {
			result = String.valueOf((char) ascii);
	...
	public String getFirstPySpelling(String chs) {
		String key, value;
		buffer = new StringBuilder();
		for (int i = 0; i < chs.length(); i++) {
			key = chs.substring(i, i + 1);
