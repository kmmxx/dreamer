...
public class GlesLoadedObject extends GlesObject {
	...
	static int maTextCoorHandle;// 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟�?
								// static int maLightDirectionHandle; //
								// 锟斤拷源位锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷

	protected Context context;
	protected boolean update = true;
	private String TAG = GlesLoadedObject.class.getSimpleName();
	int textureId = -1;
	...
	public GlesLoadedObject(Context ctx, float[] vertices) {
		super();
	...
	public GlesLoadedObject(Context ctx, float[] vertices, float[] normals,
			float[] textCoors) {
		super();
		this.vertices = vertices;
		this.context = ctx;
	...
	private void initShader(Context context) {
		...
		initVertexData();
		if (isFirst) {
			mVertexShader = GlesToolkit.loadFromAssetsFile("object_vertex.sh",
					context.getResources());
			mFragmentShader = GlesToolkit.loadFromAssetsFile("object_frag.sh",
					context.getResources());
			mProgram = GlesToolkit.createProgram(
					GlesLoadedObject.class.getSimpleName(), mVertexShader,
					mFragmentShader);
			if (mProgram == 0) {
