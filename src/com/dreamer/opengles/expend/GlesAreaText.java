...
public class GlesAreaText extends GlesImage {
	...
	private String TAG = GlesAreaText.class.getSimpleName();
	

	public GlesAreaText(Context ctx, int width, int height) {
		...
		synchronized (mutex) {
			if (text == null) {
				...
				for (int i = 0; i < size; i++) {
					mTexts.add(text.substring(i * lineSize < text.length() ? i
							* lineSize : text.length(),
							(i + 1) * lineSize < text.length() ? (i + 1)
									* lineSize : text.length()));
				}
			}
			update = true;
	...
	private void drawStrokeText(Canvas canvas, List<String> text, float x,
		...
		for (int i = 0; i < text.size(); i++) {
			if (!fontStyleNormal) {
				...
				canvas.drawText(text.get(i), textX, textY + (i * spaceY),
						mPaint);
			...
			canvas.drawText(text.get(i), textX, textY + (i * spaceY), mPaint);
